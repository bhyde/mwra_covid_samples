...
latest_pdf_url =( latest_report_url
                  %>% read_html()
                  %>% html_node('a[href$="data.pdf"]')
                  %>% html_attr('href')
                  %>% paste0(base_url, .) )
...
```
## Extract: table of days with with samples fir each region
...
(gt(tail(lines, 6)) 
  ...
  %>% tab_source_note("Samples are usually every day for both regions.")
  %>% tab_options(table.font.size=9, data_row.padding = px(2)))
```
Convert to Timeseries of samples, i.e. one sample per row.
```{r infer_samples}
samples = (  # Convert lines into a table of samples
  ...
  %>% arrange(date))
(gt(tail(samples, 6)) 
  %>% tab_header(title="Last few samples.")
