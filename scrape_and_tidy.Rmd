...
First, a disclaimer.  I know nothing.  I know little to nothing about: virology, epidemology, RStudio, and so many other things.

The Massachusetts Water Resources Agency or MWRA has a large sewage treatment facility the handles the sewage generted by much of the area around Boston.
...
if ( ! file.exists(here("data", basename(latest_xsls_url))) ){
  ...
  file.symlink(
     basename(latest_xsls_url),
     here("data", "mwra_samples.pdf")) }
```
Extract the table from the PDF the MWRA provides.
```{r extract_data}
...
```
Convert the tibble of samples on a given date, into a tsibble of samples.  One per date and region.
```{r infer_samples}
...
```{r plot_samples, fig.width = 5, fig.height = 5, include=TRUE}
